...
<% 
	int currentButton = Integer.parseInt(request.getParameter("page"));
 	...
 	if(currentButton > 1){
%>
 		<a href="vacationManager.do?page=1">처음</a>
 <%	} 
	
 	//이전 버튼이 활성화 되었을 때(현재 위치가 2 이상일 때)
 	if(currentButton > 1){
 %> 
 		<a href="vacationManager.do?page=<%=currentButton-1%>">이전</a> 
 <%	} 
 	...
 	for(int i=startButton; i<=endButton; i++){
 		if(currentButton != i){
 %> 
		<a href="vacationManager.do?page=<%=i%>">[<%=i %>]</a>	 
 <%		} 
 		else{
 %> 
 		[<b><%=i %></b>] 
 ...
 %> 
 		<a href="vacationManager.do?page=<%=currentButton+1%>">다음</a> 
 <%
 	}
 	...
 	if(currentButton < totalButton){
 %>
 		<a href="vacationManager.do?page=<%=totalButton%>">끝</a>
<%
 	}
%>
